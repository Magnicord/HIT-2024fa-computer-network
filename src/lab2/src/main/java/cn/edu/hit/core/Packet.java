...
public class Packet {
    ...
    public Packet(int seqNum, byte[] data, int dataLength, boolean eof) {
        ...
        if (data.length != CommonConfig.DATA_SIZE) {
            throw new IllegalArgumentException("数据部分Data的长度必须为" + CommonConfig.DATA_SIZE + "字节");
        }

        // 确保dataLength不超过数据的实际长度
        if (dataLength < 0 || dataLength > CommonConfig.DATA_SIZE) {
            throw new IllegalArgumentException("数据长度dataLength必须在0到" + CommonConfig.DATA_SIZE + "之间");
        }
    ...
    public static Packet fromBytes(byte[] bytes) {
        ...
        byte[] data = new byte[CommonConfig.DATA_SIZE]; // 初始化固定大小的data数组
