...
int main() {
    ...
    struct sockaddr_in server_addr,
        client_addr;  // 定义两个 sockaddr_in 结构体变量，分别表示服务器地址和客户端地址
    socklen_t addr_len = sizeof(client_addr);  // 地址长度，初始化为客户端地址的大小
    ...
    // 接收数据包
    int recv_len =
        recvfrom(sockfd, buffer, sizeof(buffer) - 1, 0, (struct sockaddr *)&client_addr, &addr_len);
    if (recv_len < 0) {
